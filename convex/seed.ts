...
const SEED_USERS = [
  ...
  {
    email: "bobtheemployee@airdev.co",
    name: "Bob Smith",
    password: "Password-123",
...
// ---------------------------------------------------------------------------
// Internal mutation — all DB writes in one transaction-safe call
// ---------------------------------------------------------------------------
...
export const seedAll = internalAction({
  ...
  handler: async (ctx) => {
    ...
    await ctx.runMutation(internal.seed._performSeed, { users: usersWithHashes });
